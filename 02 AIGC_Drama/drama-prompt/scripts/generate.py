#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""即梦剧情/角色提示词随机组合生成器（升级版：运镜必含 + 严谨度择优）。

本 skill 是 01-Aigc（jimeng-prompt）的升级版，专治两大痛点：
  ① 画面元素过多、繁杂 —— 只输出按严谨度筛选后的「最合适几个」组合，不泛滥；
  ② 维度之间违和（题材矛盾）—— 用 -hl 严谨度对同一批候选组合打分，只保留协调度最高的。

每个输出仍从 800 表 7 个维度各取 1 个值（全维度，不做子集裁剪），且每图必含 1 条随机运镜；
再用 -hl 对这批候选打分，只输出协调度最高的几个组合（分越低越协调）。

用法:
    python generate.py "一只羊"
    python generate.py "修仙者" -hl 10                 # 最严谨（硬过滤 + 大候选 + 多轮重抽）
    python generate.py "修仙者" -hl 0                   # 最松，纯随机
    python generate.py "一个东方美女" -n 5 -hl 8 --seed 42
    python generate.py "一只羊" --raw                   # 逗号拼接的原始形态
    python generate.py "一只羊" -n 3 --json             # 结构化输出
紧凑写法（与上方等价）:
    h8 n5 修仙者          # -hl 8 -n 5 修仙者
    抽3 一只羊            # -n 3 一只羊
"""
...
def normalize_argv(argv):
    ...
    VALUE_OPTIONS = {"-n", "-hl", "--count", "--hl", "--seed", "--candidates", "--retries"}
    expanded = []
    prev = None
    for token in argv:
        matched = False
...
def render(values: dict, templates: dict, rng: random.Random, raw: bool, with_suffix: bool) -> str:
    """把各维度取值渲染成一段自然语言提示词（不含运镜，运镜单独成行输出）。"""
...
# 主提示词文本使用的维度顺序（运镜单独成行，不放进主文本）
MAIN_ORDER = ["subject", "style", "lens", "scene", "detail", "tone", "culture"]


def main() -> int:
    parser = argparse.ArgumentParser(description="即梦提示词随机组合生成器（运镜必含 + 严谨度择优）")
    parser.add_argument(
        "subject",
        nargs="?",
        default=None,
    ...
    if not params or not templates:
    ...
    labels = {dim["key"]: dim["label"] for dim in dimensions}
    pools = {
        dim["key"]: dim["items"]
    ...
    character_pool = next(
        (dim["items"] for dim in dimensions if dim["key"] == "character" and dim["items"]),
        [],
    )
    ...
    # 运镜池：每图必含 1 条
    camera_list = params.get("cameras") or []
    if not camera_list:
        sys.exit("运镜数据为空，请重新执行 build_data.py")

    base_limit = min(len(pool) for pool in pools.values())
    if not auto_subject and args.count > base_limit:
        sys.exit(f"生成条数 {args.count} 超过单维度候选上限 {base_limit}，请降低数量")
    if auto_subject and args.count > len(character_pool):
        sys.exit(f"生成条数 {args.count} 超过「人物与服饰」候选上限 {len(character_pool)}，请降低数量")
    ...
    if not auto_subject and args.count > limit:
    ...
    for _ in range(rounds):
        ...
        for index in range(candidates):
            ...
            cam = rng.choice(camera_list)
            scored.append((judge.penalty(values), values, cam))

        if judge.enabled and scored and min(item[0] for item in scored) <= 0:
            break

    scored.sort(key=lambda item: item[0])
    selected = [(pen, values, cam) for pen, values, cam in scored[: args.count]]

    main_templates = dict(templates)
    main_templates["order"] = MAIN_ORDER

    results = []
    for values, cam in [(v, c) for _, v, c in selected]:
        prompt = render(values, main_templates, rng, args.raw, not args.no_suffix)
        results.append(
            {
                "prompt": prompt,
                "camera": cam,
                "penalty": judge.penalty(values),
                "values": values,
            }
        )
    ...
    best = min(item["penalty"] for item in results) if results else 0
